# Renish Cold + Hot wallet MasterNode setup guide
...
This is the wallet where the MasterNode collateral of 10.000 RNSH coins will have to be transferred and stored.
After the setup is complete, this wallet doesn't have to run 24/7 and will be the one receiving the rewards.

### 1. Install and open the Renish-Qt wallet on your machine.

#### i.    Download the newest renish-{version}-{OS}.zip wallet from https://github.com/renish/renish/releases
#### ii.   Extract the renish-qt.exe from renish-{version}-{OS}.zip (e.g. for Windows)
#### iii.  Start the new renish-qt.exe
...
#### viii. Encrypt your wallet with a long passphrase and either save it in a password manager such as keepass, or write it down and keep it safe (in a locked compartment or safe) (recommended). This passphrase is your only key to your wallet, do NOT lose it or you will lose all your RNSH. Do not let anyone steal your passcode or wallet either, just like in real life!
To encrypt the wallet, go to Settings > Encrypt wallet. Enter the passphrase, click ok. You will then have to restart the wallet and then go to Settings > Unlock Wallet and then enter the passphrase to unlock the wallet, for staking, controlling the masternode or sending your RNSH.
#### ix.   Back up your wallet.dat in case of a mistake as soon as you encrypt your wallet. Once you have encrypted the wallet, your previous backups will not work, so back it up by going to File > Backup Wallet and save the backup to more than one place. Such as a USB key or a network share. I recommend keeping your wallet and passphrase seperate so that no one can steal both. They would need both to do anything to your wallet or RNSH!
...
### 4. Send exactly 10.000 RNSH coins to the address you just copied. Double check you've got the correct address before transferring the funds.
...
>If you have given the transaction a few minutes to get a confirmation and you still do not get any output from masternode outputs it means that you have not sent EXACTLY 10.000 RNSH in a single transaction. 
...
### 5. Install the Renish CLI wallet. Always download the latest [release available](https://github.com/renish/renish/releases). The file will be renish.zip. Next, unpack it

If you are already running a `renishd` on your server and want to upgrade it, stop the current one with:
```
renishd stop
```
Run the following command until the renishd process disappears.
```
ps aux | grep renishd | grep -v grep
```
...
wget https://github.com/renish/renish/releases/download/v2.1.0/renish-2.1.0-x86_64-linux-gnu.tar.gz
tar -zxvf renish-2.1.0-x86_64-linux-gnu.tar.gz
rm renish-2.1.0-x86_64-linux-gnu.tar.gz
chmod +x ./renish-2.1.0/bin/renishd
sudo mv ./renish-2.1.0/bin/renishd /usr/local/bin
chmod +x ./renish-2.1.0/bin/renish-cli
sudo mv ./renish-2.1.0/bin/renish-cli /usr/local/bin
cd ~
renishd
```

You'll get a start error like `Error: To use renishd, or the -server option to renish-qt, you must set an rpcpassword in the configuration file`. It's expected because we haven't created the config file yet.

The service will only start for a second and create the initial data directory(`~/.renish/`).

### 6. Edit the MasterNode main wallet configuration file:
```
nano ~/.renish/renish.conf
```
...
The masternode debug log (`~/.renish/debug.log`) will contain this line on a successful activation:
```
...
RNSH - AVhZr3Sr96fVWCvKSqd6kGPWEQB9U89LUy
