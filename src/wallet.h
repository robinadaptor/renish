...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT7000IFMN = 3,
    ONLY_NONDENOMINATED_NOT7000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 RNSH at the same time
    ONLY_7000 = 5,                        // find masternode outputs including locked ones (use with caution)
...
public:
    ...
    CAmount GetLockedCredit() const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == Params().MasternodeColleteralLimxDev() * COIN) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            }
