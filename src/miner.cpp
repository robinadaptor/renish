...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("RenishMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("RenishMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("RenishMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("renish-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (chainActive.Tip()->nTime < 1529591297 || vNodes.empty() || pwallet->IsLocked() || !fMintableCoins || nReserveBalance >= pwallet->GetBalance() || !masternodeSync.IsSynced()) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("RenishMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running RenishMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
