...
// click on header
    ...
    ui->treeWidget->setColumnWidth(COLUMN_TYPE, 88);
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::RNSH;
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) coins, mapCoins) {
        ...
        BOOST_FOREACH (const COutput& out, coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show RNSH address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            if (rounds >= 0)
                itemOutput->setText(COLUMN_TYPE, strPad(QString::number(rounds), 11, " "));
            else
                itemOutput->setText(COLUMN_TYPE, strPad(QString(tr("n/a")), 11, " "));
